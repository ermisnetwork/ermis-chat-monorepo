...
export const UhmMessageSearchPanel: React.FC<MessageSearchPanelProps> = ({
  ...
  const { query, results, loading, loadingMore, handleInputChange, handleScroll, resetSearch, userMaps } =
  ...
  return (
    <div className="absolute inset-0 z-[100] bg-white dark:bg-[#1a1828] flex flex-col animate-in slide-in-from-right-full duration-300">
      ...
      {/* Results List */}
      <div className="flex-1 overflow-y-auto no-scrollbar relative bg-zinc-50 dark:bg-black/20" onScroll={handleScroll}>
        {/* Idle */}
        ...
        {!loading &&
          results.map((msg) => {
            ...
            const displayName = getUserDisplayName(
              msg.user,
              userId,
              userId ? client?.state?.users?.[userId] : undefined,
            );
            let parsedText = '';
            if (msg.text) {
