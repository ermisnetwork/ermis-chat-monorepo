...
export const UhmChannelInfoTabs: React.FC<ChannelInfoTabsProps> = React.memo(
  ({
    ...
    const getTabLabel = (tab: MediaTab) => {
      switch (tab) {
        case 'members':
          return t('chat.tabs.members', 'Thành viên');
        case 'media':
          return t('chat.tabs.media', 'Ảnh & Video');
        case 'links':
          return t('chat.tabs.links', 'Liên kết');
        case 'files':
          return t('chat.tabs.files', 'Tệp tin');
        default:
          return tab;
      }
    };
                ...
                {contentTab === 'media' && (
                  <div className="grid grid-cols-3 gap-1 px-1">
                    {mediaItems.map((item, idx) =>
                      MediaItemComponent ? (
                        <MediaItemComponent key={item.id || idx} item={item} onClick={handleMediaClick} />
                      ) : null,
                    )}
                  </div>
                )}
