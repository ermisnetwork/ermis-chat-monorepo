...
for (const patch of build.patches || []) {
  const patchPath = path.join(sdkRoot, patch.path);
  if (!existsSync(patchPath)) throw new Error(`OpenMLS patch not found: ${patch.path}`);
  assertEqual(sha256(readFileSync(patchPath)), patch.sha256, `OpenMLS patch ${patch.path} SHA-256`);
  run('git', ['apply', '--check', patchPath], openMlsDir);
  run('git', ['apply', patchPath], openMlsDir);
}
